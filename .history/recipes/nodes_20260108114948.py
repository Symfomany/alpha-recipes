...
def retrieve_recipes_node(state: RecipeState) -> RecipeState:
    query = state.get("query") or ""
    raw_docs = tools.recipes_retriever.invoke(query)
    docs: List[RetrievedDoc] = list(raw_docs)  # type: ignore
    # On retourne uniquement l’update ; LangGraph fusionne avec le state existant
    return {"retrieved_docs": docs}


def retrieve_cookbooks_node(state: RecipeState) -> RecipeState:
    query = state.get("query") or ""
    raw_docs = tools.cookbooks_retriever.invoke({"query": query})
    docs: List[RetrievedDoc] = list(raw_docs)  # type: ignore
...
def grade_retrieval_node(state: RecipeState) -> RecipeState:
    """
    Juge la qualité des docs récupérés : GOOD, BAD, AMBIGUOUS.
    """
    query = state.get("query") or ""
    ...
    state["clarification_needed"] = quality == "AMBIGUOUS"
    return state
